...
# variable GOOGLE_SDK_PYTHON_LOGGING=true
parent = "projects/python-docs-samples-kokoro"
...
```sh
$ GOOGLE_SDK_PYTHON_LOGGING=true python debug_logging_example.py
...
{"timestamp":"2024-12-11T19:40:00+00:00","severity":"DEBUG","processId":44180,"requestId":4274868307,"jsonPayload":{"request.headers":{"x-goog-api-client":["gl-python/3.12.0 gapic/1.20.0 gax/2.12.0 grpc/1.59.1 rest/2.12.0 pb/+n"],"User-Agent":["gcloud-python/1.20.0"],"X-Goog-User-Project":["<YOUR_PROJECT>"],"x-goog-request-params":["parent=projects%2F<YOUR_PROJECT>"]},"request.payload":"{\"contents\":[\"こんにちは\"],\"targetLanguageCode\":\"en-US\",\"parent\":\"projects\\/<YOUR_PROJECT>\"}"}}
...
```json
{
    ...
    "jsonPayload": {
        ...
        "request.headers": {
            ...
            "x-goog-api-client": [
                "gl-python/3.12.0 gapic/1.20.0 gax/2.11.0 grpc/1.66.0 rest/2.11.0 pb/+n"
            ],
            "User-Agent": [
                "gcloud-python/1.20.0"
            ],
            "X-Goog-User-Project": [
                "<YOUR_PROJECT>"
            ],
...
### The `GOOGLE_SDK_PYTHON_LOGGING` environment variable
...
logging the requests to `STDOUT`.

```python
import os
os.environ['GOOGLE_SDK_PYTHON_LOGGING'] = 'true'
...
The debugging code is designed to work with the Python standard `logging` module. You can
configure a logger and attach it to the library's namespace.
...
# Configure the logger for the Google Cloud client library
...
By configuring the `google.cloud` logger, you are using Python's native logging system instead of the internal one. This also opens the opportunity to extend the capabilities of logging in case you have specific needs, such as sending logs to a file or a remote logging service.

### Disabling logging for specific clients

You can control logging for specific clients by adjusting the logging level for their specific module namespace.
...
logging.getLogger('google.cloud.translate_v3').setLevel(logging.CRITICAL)

# The BigTable client will log all the requests
bigtable = bigtable_v2.BigtableClient()

# The TranslationServiceClient will not log any requests
translation = translate_v3.TranslationServiceClient()
```
...
Otherwise, please either file an issue on GitHub or ask a question on [Stack Overflow](https://stackoverflow.com/). In most cases creating a GitHub issue will result in a quicker turnaround time, but if you believe your question is likely to help other users in the future, Stack Overflow is a good option. When creating a Stack Overflow question, please use the [google-cloud-platform tag](https://stackoverflow.com/questions/tagged/google-cloud-platform) and [python tag](https://stackoverflow.com/questions/tagged/python).

Although there are multiple GitHub repositories associated with the Google Cloud Libraries, we recommend filing an issue in [https://github.com/googleapis/google-cloud-python](https://github.com/googleapis/google-cloud-python) unless you are certain that it belongs elsewhere. The maintainers may move it to a different repository where appropriate, but you will be notified of this via the email associated with your GitHub account.
